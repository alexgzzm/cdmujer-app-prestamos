import 'package:cdmujer_app_prestamos/features/auth/presentation/providers/auth_providers.dart';
import 'package:cdmujer_app_prestamos/features/new_credit/data/datasources/loan_balance_remote_data_source.dart';
import 'package:cdmujer_app_prestamos/features/new_credit/data/repositories/loan_balance_repository_impl.dart';
import 'package:cdmujer_app_prestamos/features/new_credit/domain/repositories/loan_balance_repository.dart';
import 'package:flutter_riverpod/flutter_riverpod.dart';

final Provider<LoanBalanceRepository> loanBalanceRepositoryProvider =
    Provider<LoanBalanceRepository>((Ref ref) {
  return LoanBalanceRepositoryImpl(
    dataSource: LoanBalanceRemoteDataSource(
      client: ref.watch(httpClientProvider),
    ),
  );
});
